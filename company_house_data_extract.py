...
for company in subcontractors:
    try:
        ...
        api_ext=api_ext.append(pd.Series(similar(str(company), str(api_ext.loc['title'])), index=['similarity_ratio']))
...
for co_no in numbers:
    try:
        ...
        resigned_on WANT
        '''
        officers = b[['name', 'officer_role','date_of_birth', 'appointed_on', 'address']].loc[0]
        try:
            officers= officers.append(pd.Series(b[['resigned_on']].loc[0]))
        except:
            pass
